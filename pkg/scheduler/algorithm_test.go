...
func newAssumeReserveFixture(t *testing.T, ctx context.Context, logger klog.Logger, node *v1.Node,
...
func TestAssumeAndReserve(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			f := newAssumeReserveFixture(t, ctx, logger, node1, plugin, pod1)
			...
			scheduleResult, err := f.algo.schedulePod(ctx, f.fwk, state, queuedPodInfo)
			if err != nil {
			...
			assumedPodInfo, status := f.algo.assumeAndReserve(ctx, state, f.fwk, queuedPodInfo, scheduleResult)
			...
			isAssumed, err := f.cache.IsAssumedPod(pod1)
			if err != nil {
			...
			if isPodInSnapshot(f.snapshot, node1.Name, pod1.Name) {
				t.Error("pod assumed in the cache should not be in the snapshot yet")
			}

			if tt.wantSuccess {
				if err := f.algo.unreserveAndForget(ctx, state, f.fwk, assumedPodInfo, scheduleResult.SuggestedHost); err != nil {
...
func TestAssumeAndReserveWithRevert(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			f := newAssumeReserveFixture(t, ctx, logger, node1, plugin, tt.pod)
			...
			if nominated != "" {
				f.queue.AddNominatedPod(logger, podInfo, &fwk.NominatingInfo{
			...
			status, revertFn := f.algo.assumeAndReserveWithRevert(ctx, state, f.fwk, queuedPodInfo, scheduleResult)
			if !status.IsSuccess() {
			...
			isAssumed, err := f.cache.IsAssumedPod(tt.pod)
			if err != nil {
				t.Fatalf("cache.IsAssumedPod() error: %v", err)
			}
			if isAssumed {
			...
			if !isPodInSnapshot(f.snapshot, node1.Name, tt.pod.Name) {
			...
			if nominated != "" && len(f.queue.NominatedPodsForNode(nominated)) != 0 {
...
func TestAssumeAndReserveSnapshotSync(t *testing.T) {
	...
	f := newAssumeReserveFixture(t, ctx, logger, node1, plugin, pod1, pod2)

	// pod1 goes through an ordinary cycle: assumed in the cache.
	pod1Info, err := framework.NewPodInfo(pod1)
	if err != nil {
	...
	res1, err := f.algo.schedulePod(ctx, f.fwk, state1, queuedPod1Info)
	if err != nil {
		t.Fatalf("schedulePod failed for pod1: %v", err)
	}
	if _, status := f.algo.assumeAndReserve(ctx, state1, f.fwk, queuedPod1Info, res1); !status.IsSuccess() {
		t.Fatalf("assumeAndReserve failed for pod1: %v", status)
	}

	// pod2 goes through a pod group cycle: assumed in the snapshot.
	pod2Info, err := framework.NewPodInfo(pod2)
	if err != nil {
		t.Fatalf("Failed to create PodInfo for pod2: %v", err)
	}
	...
	res2, err := f.algo.schedulePod(ctx, f.fwk, state2, queuedPod2Info)
	if err != nil {
		t.Fatalf("schedulePod failed for pod2: %v", err)
	}
	if status, _ := f.algo.assumeAndReserveWithRevert(ctx, state2, f.fwk, queuedPod2Info, res2); !status.IsSuccess() {
		t.Fatalf("assumeAndReserveWithRevert failed for pod2: %v", status)
	}

	if !isPodInSnapshot(f.snapshot, node1.Name, pod2.Name) {
		t.Error("expected pod2 to be in the snapshot before UpdateSnapshot")
	}

	if err := f.cache.UpdateSnapshot(logger, f.snapshot); err != nil {
		t.Fatalf("cache.UpdateSnapshot failed: %v", err)
	}

	if !isPodInSnapshot(f.snapshot, node1.Name, pod1.Name) {
		t.Error("expected pod1, assumed in the cache, to be visible in the snapshot after UpdateSnapshot")
	}
	if isPodInSnapshot(f.snapshot, node1.Name, pod2.Name) {
		t.Error("expected pod2, assumed in the snapshot only, to be gone after UpdateSnapshot")
	}
}
