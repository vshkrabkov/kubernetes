...
func Test_UpdatePodGroupMember(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			storedPod := cache.podGroupStates[podGroupKey].allPods[newPod.UID]
			if diff := cmp.Diff(tt.newPod, storedPod); diff != "" {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if podGroupStateCount != tt.expectPodGroupStateCount {
				t.Fatalf("expected %d pod groups, got %d", tt.expectPodGroupStateCount, podGroupStateCount)
			}
...
func Test_BindingPodGroupMember(t *testing.T) {
	...
	if !podGroupState.AllPods().Has(assumedPod.UID) {
		t.Errorf("Expected pod to remain in AllPods after binding confirmation")
	}
}
...
// TestPodGroupPodOperationsWithFeatureGate tests that operations (Add, Update, Remove, Assume, Forget) on
// pods with pod group name properly update PodGroupStates only when GenericWorkload feature gate is enabled.
func TestPodGroupPodOperationsWithFeatureGate(t *testing.T) {
	groupName := "pg"
	...
	tests := []struct {
		...
		},
		{
			...
			expected: state{podGroupStatesCount: 1, allPodsCount: 1, assignedCount: 0, unscheduledCount: 1, assumedCount: 0},
		},
		{
		...
		},
		{
			...
			expected: state{podGroupStatesCount: 1, allPodsCount: 1, assignedCount: 0, unscheduledCount: 0, assumedCount: 1},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allPodsCount := pgs.AllPods().Len()
			if allPodsCount != tt.expected.allPodsCount {
				t.Errorf("expected %d pods in allPods, got %d", tt.expected.allPodsCount, allPodsCount)
			}
