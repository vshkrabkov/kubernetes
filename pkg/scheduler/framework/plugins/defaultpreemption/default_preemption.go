...
type IsEligiblePreemptorFunc func(domain preemption.Domain, victim preemption.Victim, preemptor preemption.Preemptor) bool
...
func New(_ context.Context, dpArgs runtime.Object, fh fwk.Handle, fts feature.Features) (*DefaultPreemption, error) {
	...
	pl.IsEligiblePreemptor = func(domain preemption.Domain, victim preemption.Victim, preemptor preemption.Preemptor) bool {
		return true
	}
...
func (pl *DefaultPreemption) SelectVictimsOnDomain(
	...
	removePods := func(pu preemption.Victim) error {
	...
	addPods := func(pu preemption.Victim) error {
		for _, pi := range pu.Pods() {
			nodeInfo := nameToNode[pi.GetPod().Spec.NodeName]
	...
	if len(nomNodeName) > 0 {
		...
		if nodeInfo, _ := nodeInfos.Get(nomNodeName); nodeInfo != nil {
			for _, p := range nodeInfo.GetPods() {
				preemptionUnit := preemption.NewVictim([]fwk.PodInfo{p}, corev1helpers.PodPriority(p.GetPod()), []fwk.NodeInfo{nodeInfo})
