		...
		if !preemptedInMemory {
			...
			if updated {
				if err := util.PatchPodStatus(ctx, fh.ClientSet(), victim.Name, victim.Namespace, victim.UID, &victim.Status, newStatus); err != nil {
					if !apierrors.IsNotFound(err) && !util.IsPodUIDMismatch(err) {
					...
					// The victim is gone, or its name was taken over by a Pod that was never
					// selected for preemption. Return before DeletePod, which addresses the
					// Pod by name only and would otherwise delete the wrong object.
